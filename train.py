...
class Train(object):
    ...
    def __init__(self, sess, learning_rate, data_sets_path, batch_size, canvas_size, window_size, threads, max_steps,
                 save_path, optimizer, kwidth, stride, is_train, beta1, summary_step, saver_step, order):
        ...
        self.wav_data = reader.Reader(data_sets_path, canvas_size, batch_size, window_size,
                                      threads).get_batch()
    ...
    def train_on_gpu(self):
        ...
        wav_data_reshape = tf.reshape(wav_data, [-1, self.window_size])
        generate = inference.Inference(wav_data_reshape, self.kwidth, self.stride, self.is_train)

        generator = generate.build_seae_model()

        lpca = tf.placeholder(tf.float32, [None, self.order], name="lpca")

        loss = losses.Losses(generator, lpca, wav_data_reshape).get_loss()
        ...
        for i in xrange(self.max_step):
            ...
            train_collect = self.sess.run(self.wav_data)
            print np.shape(train_collect)
            generate_data = generator.eval(feed_dict={
                wav_data: train_collect
            })

            lpca_data = generate.get_lpc_a(generate_data, self.order)
